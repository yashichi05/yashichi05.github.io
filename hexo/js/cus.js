...
window.addEventListener('DOMContentLoaded', () => {
  //決定搜尋input高度
  const search_top_calc = () => {
    let search_top = document.querySelector(".title").getClientRects()[0].top + document.querySelector(".title").offsetHeight;
    document.querySelector("#layout_search").style.top = search_top + "px";
    let res_top = document.querySelector("header").offsetHeight-search_top;
    document.querySelector("#search-result").style.top = res_top + "px"
  }
  search_top_calc();
  window.onresize = search_top_calc;
  ...
  const search_tool = document.getElementById('search-tool');
  ...
  const inputEventFunction = () => {
    ...
    if (keywords.length === 1 && keywords[0] === '') {
      ...
      resultContent.innerHTML = '<div id="no-result"><ul><li><p class="search-result noResult">查無結果</p></li></ul></div>';
    } else {
  ...
  const inputFocusinFunction = () => {
    resultContent.style.display = '';
  };
  const inputFocusoutFunction = () => {
    resultContent.style.display = 'none';
  };
  if (CONFIG.localsearch.trigger === 'auto') {
    search_tool.addEventListener('focusin', inputFocusinFunction);

    document.addEventListener('click', function (event) {
      var isClickInside = search_tool.contains(event.target); //點取物件不在search_tool
      if (!isClickInside) {
        inputFocusoutFunction();
      }
    });
